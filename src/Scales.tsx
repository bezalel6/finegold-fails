...
function animate(func: () => void, ms: number = 500) {
    ...
    destroyer.push(() => { clearInterval(interval) })
}
...
class ScalesState {
    ...
    lastLeft = 0;
    updateWeights() {
        const r = this.rightItems, l = this.leftItems;
...
class MyElement<T extends HTMLElement = HTMLImageElement> {
    ...
    addChild(child: Node) {
        ...
        if (this.containedElements)
            this.containedElements.addChild(child)
        else {
    ...
    translate(operation: Operation | string) {
        const op: Operation = typeof operation === "string" ? { type: 'translate', amount: operation, replaceOldOps: false } as Operation : operation;
